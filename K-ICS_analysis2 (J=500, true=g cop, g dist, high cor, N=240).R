...
#### VaR estimation with sample size of 240 (one can replace 240 with any number, say, 240, 240, or 480)####
...
for (i in 1:J) {
  ...
  sam240.risk_pnc  <- qnorm(    sam240.cop[,2], mean=500, sd=1000) - 500
  sam240.risk_cred <- qnorm(    sam240.cop[,3], mean=500, sd=1000) - 500
  sam240.risk_mkt  <- qnorm(    sam240.cop[,4], mean=500, sd=1000) - 500
  ...
  est240.parm_pnc  <- c(0, sd(sam240.risk_pnc))
  pnc_trans       <- -mean(sam240.risk_pnc)
  trs240.risk_pnc  <- sam240.risk_pnc+pnc_trans 
  
  est240.parm_cred  <- c(0, sd(sam240.risk_cred))
  cred_trans       <- -mean(sam240.risk_cred)
  trs240.risk_cred  <- sam240.risk_cred+cred_trans 
  
  est240.parm_mkt  <- c(0, sd(sam240.risk_mkt))
  mkt_trans       <- -mean(sam240.risk_mkt)
  trs240.risk_mkt  <- sam240.risk_mkt+mkt_trans 
  
  
  prd240.var_marginal <- c(
    qnorm(    0.995, sd   =est240.parm_life[2], mean=est240.parm_life[1]),
    qnorm(    0.995, sd   =est240.parm_pnc[ 2], mean=est240.parm_pnc[ 1]),
    qnorm(    0.995, sd   =est240.parm_cred[2], mean=est240.parm_cred[1]),
    qnorm(    0.995, sd   =est240.parm_mkt[ 2], mean=est240.parm_mkt[ 1]))
  
  psd240.cop <- cbind(
    pnorm(      trs240.risk_life, mean=est240.parm_life[1], sd =est240.parm_life[2]),
    pnorm(      trs240.risk_pnc , mean=est240.parm_pnc[ 1], sd =est240.parm_pnc[ 2]),
    pnorm(      trs240.risk_cred, mean=est240.parm_cred[1], sd =est240.parm_cred[2]),
    pnorm(      trs240.risk_mkt , mean=est240.parm_mkt[ 1], sd =est240.parm_mkt[ 2]))
  ...
  init.elpparm <- c(cor(psd240.cop)[1,2:4], cor(psd240.cop)[2,3:4], cor(psd240.cop)[3,4])
  ...
  c2vm <- RVineStructureSelect(psd240.cop,
                               rotations = T, 
                               selectioncrit = "AIC",
                               indeptest = TRUE, level = 0.05, 
                               type = "RVine")
  ...
  prd240.elpcopfit <- fitCopula(tCopula(dim=4, dispstr = "un", df=4, df.fixed = TRUE), psd240.cop, method="mpl",
                               start=init.elpparm)
  ...
  pld240.risk_pnc  <- qnorm(    pld240.cop[,2], sd   =est240.parm_pnc[2], mean=est240.parm_pnc[1])
  pld240.risk_pnc  <- pld240.risk_pnc  - mean(pld240.risk_pnc)
  pld240.risk_cred <- qnorm(    pld240.cop[,3], sd   =est240.parm_cred[2], mean=est240.parm_cred[1])
  pld240.risk_cred <- pld240.risk_cred  - mean(pld240.risk_cred)
  pld240.risk_mkt  <- qnorm(    pld240.cop[,4], sd   =est240.parm_mkt[2], mean=est240.parm_mkt[1])
  ...
  pad240.risk_pnc  <- qnorm(    pad240.cop[,2], sd   =est240.parm_pnc[2], mean=est240.parm_pnc[1])
  pad240.risk_pnc  <- pad240.risk_pnc  - mean(pad240.risk_pnc)
  pad240.risk_cred <- qnorm(    pad240.cop[,3], sd   =est240.parm_cred[2], mean=est240.parm_cred[1])
  pad240.risk_cred <- pad240.risk_cred  - mean(pad240.risk_cred)
  pad240.risk_mkt  <- qnorm(    pad240.cop[,4], sd   =est240.parm_mkt[2], mean=est240.parm_mkt[1])
  ...
  pcv240.risk_life <- pcv240.risk_life - mean(pad240.risk_life)
  pcv240.risk_pnc  <- qnorm(    pcv240.cop[,2], sd   =est240.parm_pnc[2], mean=est240.parm_pnc[1])
  pcv240.risk_pnc  <- pcv240.risk_pnc  - mean(pad240.risk_pnc)
  pcv240.risk_cred <- qnorm(    pcv240.cop[,3], sd   =est240.parm_cred[2], mean=est240.parm_cred[1])
  pcv240.risk_cred <- pcv240.risk_cred  - mean(pad240.risk_cred)
  pcv240.risk_mkt  <- qnorm(    pcv240.cop[,4], sd   =est240.parm_mkt[2], mean=est240.parm_mkt[1])
  ...
  pc2v240.cop <- RVineSim(20000, c2vm)
  
  pc2v240.risk_life <- qnorm(    pc2v240.cop[,1], sd   =est240.parm_life[2], mean=est240.parm_life[1])
  pc2v240.risk_life <- pc2v240.risk_life - mean(pad240.risk_life)
  pc2v240.risk_pnc  <- qnorm(    pc2v240.cop[,2], sd   =est240.parm_pnc[2], mean=est240.parm_pnc[1])
  pc2v240.risk_pnc  <- pc2v240.risk_pnc  - mean(pad240.risk_pnc)
  pc2v240.risk_cred <- qnorm(    pc2v240.cop[,3], sd   =est240.parm_cred[2], mean=est240.parm_cred[1])
  pc2v240.risk_cred <- pc2v240.risk_cred  - mean(pad240.risk_cred)
  pc2v240.risk_mkt  <- qnorm(    pc2v240.cop[,4], sd   =est240.parm_mkt[2], mean=est240.parm_mkt[1])
  pc2v240.risk_mkt  <- pc2v240.risk_mkt  - mean(pc2v240.risk_mkt)
  pc2v240.risk_total <- pc2v240.risk_life + pc2v240.risk_pnc + pc2v240.risk_cred + pc2v240.risk_mkt
  ...
  prd240.c2vine[i] <- quantile(pc2v240.risk_total, probs=0.995)
  ...
  prd240.var_C2Vine <- prd240.var_C2Vine + quantile(pc2v240.risk_total, probs=0.995)/J
}

c(prd240.var_std, prd240.var_emp, prd240.var_arc, prd240.var_elp, prd240.var_CVine, prd240.var_C2Vine)
c(prd240.var_std, prd240.var_emp, prd240.var_arc, prd240.var_elp, prd240.var_CVine, prd240.var_C2Vine) - pop.var_true
...
summary(prd240.c2vine - pop.var_true)

save.image("K-ICS_analysis2 (J=500, true=g cop, g dist, high cor, N=240).RData")
