...
for (i in 1:J) {
  ...
  pc2v240.risk_life <- pc2v240.risk_life - mean(pc2v240.risk_life)
  pc2v240.risk_pnc  <- qpareto2( pc2v240.cop[,2], s    =est240.parm_pnc[ 2], k   =est240.parm_pnc[ 1]) 
  pc2v240.risk_pnc  <- pc2v240.risk_pnc  - mean(pc2v240.risk_pnc)
  pc2v240.risk_cred <- qt(       pc2v240.cop[,3], df=4)*est240.parm_cred[2]      +est240.parm_cred[1] 
  pc2v240.risk_cred <- pc2v240.risk_cred  - mean(pc2v240.risk_cred)
  pc2v240.risk_mkt  <- exp(qnorm(pc2v240.cop[,4], sd   =est240.parm_mkt[ 2], mean=est240.parm_mkt[ 1]))
...
save.image("K-ICS_analysis2 (J=500, true=t cop, 4 dist, high cor, N=240).RData")
